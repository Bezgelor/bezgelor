defmodule BezgelorProtocol.ValidationTest do
  ...
  describe "validate_position/1" do
    ...
    test "rejects position out of bounds" do
      assert {:error, :position_out_of_bounds} = Validation.validate_position({999_999.0, 0.0, 0.0})
      assert {:error, :position_out_of_bounds} = Validation.validate_position({0.0, 999_999.0, 0.0})
      assert {:error, :position_out_of_bounds} = Validation.validate_position({0.0, 0.0, 999_999.0})
      assert {:error, :position_out_of_bounds} = Validation.validate_position({-999_999.0, 0.0, 0.0})
    end
    ...
    test "rejects wrong tuple size" do
      ...
      assert {:error, :invalid_position_type} = Validation.validate_position({100.0, 50.0, 200.0, 300.0})
    end
  end
  ...
  describe "is_nan_or_inf/1" do
    ...
    test "returns false for regular floats" do
      ...
      refute Validation.is_nan_or_inf(-100.5)
      refute Validation.is_nan_or_inf(1.7976931348623157e308)  # Max float
    end
  ...
  describe "validate_enum/2" do
    ...
    test "rejects value not in allowed set" do
      ...
      assert {:error, {:invalid_enum, :qux, [:foo, :bar]}} = Validation.validate_enum(:qux, [:foo, :bar])
    end
  end
  ...
  describe "validate_range/3" do
    test "accepts value in range" do
      ...
      assert :ok = Validation.validate_range(1, 1, 10)  # Lower bound
      assert :ok = Validation.validate_range(10, 1, 10)  # Upper bound
    end
  ...
  describe "validate_all/1" do
    ...
    test "short-circuits on first error" do
      result = Validation.validate_all([
        {&Validation.validate_string/1, [123]},  # This fails first
        {&Validation.validate_position/1, [{999_999.0, 0.0, 0.0}]}
      ])
  ...
  describe "validate_faction/1" do
    test "accepts valid factions" do
      assert :ok = Validation.validate_faction(166)  # Exile
      assert :ok = Validation.validate_faction(167)  # Dominion
    end
