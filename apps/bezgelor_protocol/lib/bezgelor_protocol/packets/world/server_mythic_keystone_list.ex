defmodule BezgelorProtocol.Packets.World.ServerMythicKeystoneList do
  ...
  def write(%__MODULE__{} = packet, writer) do
    writer = PacketWriter.write_u8(writer, length(packet.keystones))
    ...
    writer =
      Enum.reduce(packet.keystones, writer, fn keystone, w ->
        w = w
          ...
          |> PacketWriter.write_u8(if(keystone.depleted, do: 1, else: 0))
          |> PacketWriter.write_u8(length(keystone.affix_ids))

        Enum.reduce(keystone.affix_ids, w, fn affix_id, w2 ->
          PacketWriter.write_u32(w2, affix_id)
        end)
      end)
