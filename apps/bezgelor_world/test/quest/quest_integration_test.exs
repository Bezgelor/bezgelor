defmodule BezgelorWorld.Quest.IntegrationTest do
  ...
  describe "quest lifecycle integration" do
    ...
    test "quest does not over-increment past target" do
      ...
      assert get_objective_progress(session_data, 100, 0) == 3
      # No packet since nothing changed
      assert packets == []
    end
  ...
  describe "event type coverage" do
    test "interact event increments interact_object objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # interact_object
          objective: %{index: 0, current: 0, target: 1, type: 12, data: 789}
        )
    ...
    test "enter_location event increments location objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # enter_location
          objective: %{index: 0, current: 0, target: 1, type: 5, data: 1234}
        )
    ...
    test "talk_npc event increments talk_to_npc objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # talk_to_npc
          objective: %{index: 0, current: 0, target: 1, type: 4, data: 555}
        )
    ...
    test "use_item event increments use_item objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # use_item
          objective: %{index: 0, current: 0, target: 1, type: 8, data: 2001}
        )
    ...
    test "use_ability event increments use_ability objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # use_ability
          objective: %{index: 0, current: 0, target: 1, type: 17, data: 3000}
        )
    ...
    test "gather event increments gather_resource objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # gather_resource
          objective: %{index: 0, current: 0, target: 1, type: 33, data: 4000}
        )
    ...
    test "complete_event event increments complete_event objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # complete_event
          objective: %{index: 0, current: 0, target: 1, type: 31, data: 5000}
        )
    ...
    test "condition_met event increments achieve_condition objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # achieve_condition
          objective: %{index: 0, current: 0, target: 1, type: 32, data: 6000}
        )
      ...
      session_data = %{
        active_quests: %{
          100 => %{
            ...
            objectives: [
              # Kill creature 100
              %{index: 0, current: 0, target: 2, type: 2, data: 100},
              # Collect item 200
              %{index: 1, current: 0, target: 3, type: 3, data: 200},
              # Interact object 300
              %{index: 2, current: 0, target: 1, type: 12, data: 300}
            ],
            dirty: false
          }
        },
      ...
      session_data = %{
        active_quests: %{
          100 => %{
            ...
            objectives: [
              # 1/2 kills
              %{index: 0, current: 1, target: 2, type: 2, data: 100},
              # 2/2 items (complete)
              %{index: 1, current: 2, target: 2, type: 3, data: 200}
            ],
            dirty: false
          }
        },
  ...
  describe "edge cases" do
    ...
    test "completed quests ignore events" do
      ...
      # No change
      assert updated[:active_quests][100].objectives ==
               session_data[:active_quests][100].objectives
    ...
    test "deliver_item event increments deliver_item objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # deliver_item
          objective: %{index: 0, current: 0, target: 1, type: 6, data: 1001}
        )
    ...
    test "equip_item event increments equip_item objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # equip_item
          objective: %{index: 0, current: 0, target: 1, type: 7, data: 2002}
        )
    ...
    test "craft event increments craft_item objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # craft_item
          objective: %{index: 0, current: 0, target: 3, type: 13, data: 3003}
        )
    ...
    test "datacube event increments activate_datacube objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # activate_datacube
          objective: %{index: 0, current: 0, target: 1, type: 20, data: 4004}
        )
    ...
    test "scan event increments scan_creature objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # scan_creature
          objective: %{index: 0, current: 0, target: 1, type: 21, data: 5005}
        )
    ...
    test "explore event increments explore objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # explore
          objective: %{index: 0, current: 0, target: 1, type: 25, data: 6006}
        )
    ...
    test "discover_poi event increments discover_poi objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # discover_poi
          objective: %{index: 0, current: 0, target: 1, type: 15, data: 7007}
        )
    ...
    test "escort_complete event increments escort_npc objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # escort_npc
          objective: %{index: 0, current: 0, target: 1, type: 14, data: 8008}
        )
    ...
    test "defend_complete event increments defend_location objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # defend_location
          objective: %{index: 0, current: 0, target: 1, type: 18, data: 9009}
        )
    ...
    test "dungeon_complete event increments complete_dungeon objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # complete_dungeon
          objective: %{index: 0, current: 0, target: 1, type: 9, data: 1010}
        )
    ...
    test "sequence_step event increments objective_sequence objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # objective_sequence
          objective: %{index: 0, current: 0, target: 3, type: 11, data: 1111}
        )
    ...
    test "timed_complete event increments timed_event objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # timed_event
          objective: %{index: 0, current: 0, target: 1, type: 16, data: 1212}
        )
    ...
    test "path_complete event increments path_mission objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # path_mission
          objective: %{index: 0, current: 0, target: 1, type: 19, data: 1313}
        )
    ...
    test "pvp_win event increments win_pvp objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # win_pvp
          objective: %{index: 0, current: 0, target: 1, type: 28, data: 1414}
        )
    ...
    test "pvp_win with zero data matches any battleground" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # win_pvp any
          objective: %{index: 0, current: 0, target: 3, type: 28, data: 0}
        )
    ...
    test "capture event increments capture_point objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # capture_point
          objective: %{index: 0, current: 0, target: 3, type: 29, data: 1515}
        )
    ...
    test "challenge_complete event increments challenge objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # challenge
          objective: %{index: 0, current: 0, target: 1, type: 35, data: 1616}
        )
    ...
    test "reputation_gain event increments reputation objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # reputation
          objective: %{index: 0, current: 0, target: 1, type: 36, data: 1717}
        )
    ...
    test "level_up event increments level_requirement objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # level_requirement
          objective: %{index: 0, current: 0, target: 1, type: 37, data: 20}
        )
    ...
    test "level_up event does not increment if level too low" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # requires level 30
          objective: %{index: 0, current: 0, target: 1, type: 37, data: 30}
        )
    ...
    test "achievement event increments achievement objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # achievement
          objective: %{index: 0, current: 0, target: 1, type: 44, data: 1818}
        )
    ...
    test "title event increments title objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # title
          objective: %{index: 0, current: 0, target: 1, type: 42, data: 1919}
        )
    ...
    test "housing event increments housing objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # housing
          objective: %{index: 0, current: 0, target: 1, type: 39, data: 2020}
        )
    ...
    test "mount event increments mount objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # mount
          objective: %{index: 0, current: 0, target: 1, type: 40, data: 2121}
        )
    ...
    test "costume event increments costume objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # costume
          objective: %{index: 0, current: 0, target: 1, type: 41, data: 2222}
        )
    ...
    test "currency event increments currency objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # currency
          objective: %{index: 0, current: 0, target: 1, type: 46, data: 2323}
        )
    ...
    test "social event increments social objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # social
          objective: %{index: 0, current: 0, target: 1, type: 47, data: 2424}
        )
    ...
    test "guild event increments guild objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # guild
          objective: %{index: 0, current: 0, target: 1, type: 48, data: 2525}
        )
    ...
    test "special event increments special objectives" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # special
          objective: %{index: 0, current: 0, target: 1, type: 27, data: 2626}
        )
    ...
    test "generic objectives match kill events" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # generic
          objective: %{index: 0, current: 0, target: 1, type: 38, data: 456}
        )
    ...
    test "generic objectives match loot events" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # generic
          objective: %{index: 0, current: 0, target: 1, type: 38, data: 789}
        )
    ...
    test "generic objectives match interact events" do
      session_data =
        build_session_with_quest(
          quest_id: 100,
          # generic
          objective: %{index: 0, current: 0, target: 1, type: 38, data: 1000}
        )
